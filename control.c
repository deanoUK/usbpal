...
static void usbpal_control_output_vol_update(struct control_runtime *rt)
{
...
static void usbpal_control_output_mute_update(struct control_runtime *rt)
{
	struct comm_runtime *comm_rt = rt->chip->comm;
...
static void usbpal_control_input_vol_update(struct control_runtime *rt)
{
	struct comm_runtime *comm_rt = rt->chip->comm;
	int i;
...
static void usbpal_control_line_phono_update(struct control_runtime *rt)
{
	struct comm_runtime *comm_rt = rt->chip->comm;
	if (comm_rt) {
...
static void usbpal_control_opt_coax_update(struct control_runtime *rt)
{
	struct comm_runtime *comm_rt = rt->chip->comm;
	if (comm_rt) {
...
static int usbpal_control_set_rate(struct control_runtime *rt, int rate)
{
	int ret;
...
static int usbpal_control_set_channels(
...
static int usbpal_control_streaming_update(struct control_runtime *rt)
{
	struct comm_runtime *comm_rt = rt->chip->comm;

	if (comm_rt) {
		if (!rt->usb_streaming && rt->digital_thru_switch)
			usbpal_control_set_rate(rt,
...
static int usbpal_control_output_vol_info(struct snd_kcontrol *kcontrol,
...
static int usbpal_control_output_vol_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (changed)
		usbpal_control_output_vol_update(rt);

	return changed;
}

static int usbpal_control_output_vol_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
	unsigned int ch = kcontrol->private_value;
...
static int usbpal_control_output_mute_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
	unsigned int ch = kcontrol->private_value;
	...
	if (rt->output_mute != old)
		usbpal_control_output_mute_update(rt);

	return rt->output_mute != old;
}

static int usbpal_control_output_mute_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
	unsigned int ch = kcontrol->private_value;
...
static int usbpal_control_input_vol_info(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->count = 2;
...
static int usbpal_control_input_vol_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
	int changed = 0;
	...
	if (changed)
		usbpal_control_input_vol_update(rt);

	return changed;
}

static int usbpal_control_input_vol_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
...
static int usbpal_control_line_phono_info(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_info *uinfo)
{
...
static int usbpal_control_line_phono_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
	int changed = 0;
	if (rt->line_phono_switch != ucontrol->value.integer.value[0]) {
		...
		usbpal_control_line_phono_update(rt);
		changed = 1;
	}
	return changed;
}

static int usbpal_control_line_phono_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
...
static int usbpal_control_opt_coax_info(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_ENUMERATED;
	uinfo->count = 1;
...
static int usbpal_control_opt_coax_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
	int changed = 0;
	...
	if (rt->opt_coax_switch != ucontrol->value.enumerated.item[0]) {
		...
		usbpal_control_opt_coax_update(rt);
		changed = 1;
	}
	return changed;
}

static int usbpal_control_opt_coax_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
...
static int usbpal_control_digital_thru_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
	int changed = 0;
	...
	if (rt->digital_thru_switch != ucontrol->value.integer.value[0]) {
		...
		usbpal_control_streaming_update(rt);
		changed = 1;
	}
	return changed;
}

static int usbpal_control_digital_thru_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct control_runtime *rt = snd_kcontrol_chip(kcontrol);
...
static struct __devinitdata snd_kcontrol_new vol_elements[] = {
	{
		...
		.info = usbpal_control_output_vol_info,
		.get = usbpal_control_output_vol_get,
		.put = usbpal_control_output_vol_put,
		.tlv = { .p = tlv_output }
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...
static struct __devinitdata snd_kcontrol_new mute_elements[] = {
	{
		...
		.get = usbpal_control_output_mute_get,
		.put = usbpal_control_output_mute_put,
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		.name = "Analog Playback Switch",
...
static struct __devinitdata snd_kcontrol_new elements[] = {
	{
		...
		.index = 0,
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
		.info = usbpal_control_line_phono_info,
		.get = usbpal_control_line_phono_get,
		.put = usbpal_control_line_phono_put
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		...
		.index = 0,
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE,
		.info = usbpal_control_opt_coax_info,
		.get = usbpal_control_opt_coax_get,
		.put = usbpal_control_opt_coax_put
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		...
		.get = usbpal_control_digital_thru_get,
		.put = usbpal_control_digital_thru_put
	},
	{
		.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
		...
		.access = SNDRV_CTL_ELEM_ACCESS_READWRITE |
			SNDRV_CTL_ELEM_ACCESS_TLV_READ,
		.info = usbpal_control_input_vol_info,
		.get = usbpal_control_input_vol_get,
		.put = usbpal_control_input_vol_put,
		.tlv = { .p = tlv_input }
	},
	{}
};

static int usbpal_control_add_virtual(
...
int __devinit usbpal_control_init(struct usbpal_chip *chip)
{
	...
	rt->chip = chip;
	rt->update_streaming = usbpal_control_streaming_update;
	rt->set_rate = usbpal_control_set_rate;
	rt->set_channels = usbpal_control_set_channels;
	...
	ret = usbpal_control_add_virtual(rt, chip->card,
