 ...
 *
 * CAUTION: keep sizeof <= buffer[] in usbpal_fw_init
 */
...
static u8 usbpal_fw_ihex_hex(const u8 *data, u8 *crc)
{
	u8 val = 0;
	int hval;
 ...
 */
static bool usbpal_fw_ihex_next_record(struct ihex_record *record)
{
	u8 crc = 0;
	u8 type;
	int i;
	...
	record->len = usbpal_fw_ihex_hex(record->txt_data +
			record->txt_offset, &crc);
	record->txt_offset += 2;
	record->address = usbpal_fw_ihex_hex(record->txt_data +
			record->txt_offset, &crc) << 8;
	record->txt_offset += 2;
	record->address |= usbpal_fw_ihex_hex(record->txt_data +
			record->txt_offset, &crc);
	record->txt_offset += 2;
	type = usbpal_fw_ihex_hex(record->txt_data +
			record->txt_offset, &crc);
	record->txt_offset += 2;
	...
	for (i = 0; i < record->len; i++) {
		record->data[i] = usbpal_fw_ihex_hex(record->txt_data
				+ record->txt_offset, &crc);
		record->txt_offset += 2;
	}
	usbpal_fw_ihex_hex(record->txt_data + record->txt_offset, &crc);
	if (crc) {
		record->error = true;
		return false;
	}
...
static int usbpal_fw_ihex_init(const struct firmware *fw,
		struct ihex_record *record)
{
	...
	while (usbpal_fw_ihex_next_record(record))
...
static int usbpal_fw_ezusb_write(struct usb_device *device,
...
static int usbpal_fw_ezusb_read(struct usb_device *device,
		int type, int value, char *data, int len)
{
...
static int usbpal_fw_fpga_write(struct usb_device *device,
		char *data, int len)
{
	int actual_len;
	int ret;
...
static int usbpal_fw_ezusb_upload(
		...
		unsigned int postaddr, u8 *postdata, unsigned int postlen)
{
	...
	ret = usbpal_fw_ihex_init(fw, rec);
	if (ret < 0) {
		kfree(rec);
		release_firmware(fw);
	...
	ret = usbpal_fw_ezusb_write(device, 0xa0, 0xe600, &data, 1);
	if (ret < 0) {
		kfree(rec);
		release_firmware(fw);
	...
	while (usbpal_fw_ihex_next_record(rec)) { /* write firmware */
		ret = usbpal_fw_ezusb_write(device, 0xa0, rec->address,
				rec->data, rec->len);
		if (ret < 0) {
			kfree(rec);
			release_firmware(fw);
	...
	if (postdata) { /* write data after firmware has been uploaded */
		ret = usbpal_fw_ezusb_write(device, 0xa0, postaddr,
				postdata, postlen);
		if (ret < 0) {
			snd_printk(KERN_ERR PREFIX "unable to upload ezusb "
					"firmware %s: post urb.\n", fwname);
...
static int usbpal_fw_fpga_upload(
		struct usb_interface *intf, const char *fwname)
{
	int ret;
	int i;
	...
	ret = usbpal_fw_ezusb_write(device, 8, 0, NULL, 0);
	if (ret < 0) {
		kfree(buffer);
		release_firmware(fw);
	...
	while (c != end) {
		...
		ret = usbpal_fw_fpga_write(device, buffer, i);
		if (ret < 0) {
			release_firmware(fw);
			kfree(buffer);
	...
	ret = usbpal_fw_ezusb_write(device, 9, 0, NULL, 0);
	if (ret < 0) {
		snd_printk(KERN_ERR PREFIX "unable to upload fpga firmware: "
				"end urb.\n");
		return ret;
 ...
 * info data. */
static int usbpal_fw_check(u8 *version)
{
	int i;
...
int usbpal_fw_init(struct usb_interface *intf)
{
	int i;
	int ret;
	struct usb_device *device = interface_to_usbdev(intf);
	...
	ret = usbpal_fw_ezusb_read(device, 1, 0, buffer, 8);
	if (ret < 0) {
	...
	if (buffer[3] == 0x01) {
		ret = usbpal_fw_ezusb_upload(intf,
				"usbpal/dmx6firel2.ihx", 0, NULL, 0);
		if (ret < 0)
			return ret;
		return FW_NOT_READY;
	}
	/* do we need fpga firmware and application ezusb firmware? */
		...
		ret = usbpal_fw_check(buffer + 4);
		if (ret < 0)
			return ret;
		ret = usbpal_fw_fpga_upload(intf, "usbpal/dmx6firecf.bin");
		if (ret < 0)
			return ret;
		memcpy(buffer, ep_w_max_packet_size,
				sizeof(ep_w_max_packet_size));
		ret = usbpal_fw_ezusb_upload(intf, "usbpal/dmx6fireap.ihx",
	...
	/* all fw loaded? */
		...
		return usbpal_fw_check(buffer + 4);
	/* unknown data? */
	else {
		snd_printk(KERN_ERR PREFIX "unknown device firmware state "
				"received from device: ");
